...
import type { VendorShop } from "@/lib/types/vendor";
...
export default function SignupPage() {
    ...
    // Live Shops for Dropdown
    const [shopsList, setShopsList] = useState<VendorShop[]>([]);
    const [loadingShops, setLoadingShops] = useState(true);

    useEffect(() => {
        const fetchShops = async () => {
            try {
                const fetched = await getAllShops();
                // Filter out demo-shop if needed, or keep it for testing
                setShopsList(fetched);
                if (fetched.length > 0) {
                    setShopName(fetched[0].name); // Select first by default
                }
            } catch (err) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            setLoading(true);
            await signup(email, password, fullName, "vendor_staff", shopName, staffRole as any);
            setShowSuccess(true);
    ...
    const handleSocialLogin = async (provider: string) => {
        ...
        setLoading(true);
        try {
            localStorage.setItem("redirectAfterLogin", "/vendor/staff");
            let result;
            if (provider === "google") {
            ...
            if (result) {
                ...
                setShowSuccess(true);
                setTimeout(() => router.push("/vendor/staff"), 1500);
            }
        ...
        <div className="min-h-screen bg-gray-50 flex items-center justify-center p-0 md:p-6 font-sans selection:bg-indigo-100">
            <div className="w-full max-w-md bg-white min-h-screen md:min-h-fit md:rounded-[2rem] md:shadow-2xl md:border border-gray-100 flex flex-col px-6 py-6 sm:py-8 mx-auto relative">
                ...
                <form className="space-y-5 w-full" onSubmit={handleSubmit}>
                    ...
                    {/* Shop Name Input */}
                    <div className={`flex items-center bg-[#FAFAFA] rounded-[18px] px-5 py-4 border-2 border-transparent focus-within:${primaryBorderGreen}/50 transition-all shadow-sm hover:shadow-md`}>
                        <svg className="w-5 h-5 text-gray-400 mr-4 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                        ...
                        <select
                            ...
                            className="bg-transparent border-none outline-none text-gray-900 w-full font-medium text-[15px] appearance-none disabled:opacity-50"
                            required
                        >
                            {loadingShops ? (
                    ...
                    <div className={`flex items-center bg-[#FAFAFA] rounded-[18px] px-5 py-4 border-2 border-transparent focus-within:${primaryBorderGreen}/50 transition-all shadow-sm hover:shadow-md`}>
                        ...
                        <select
                            ...
                            className="bg-transparent border-none outline-none text-gray-900 w-full font-medium text-[15px] appearance-none"
                            required
                        >
