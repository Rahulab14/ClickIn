...
export default function LoginPage() {
    const { login, googleSignIn, facebookSignIn, appleSignIn } = useAuth();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            try {
                ...
                if (staffDoc.exists()) {
                    role = "vendor_staff";
                    status = staffDoc.data().status || "active";
                } else {
                ...
                setTimeout(() => {
                    ...
                    } else if (role === "vendor_owner" || role === "vendor") {
                        if (status === "pending") {
                            router.push("/vendor/pending-approval");
                        } else {
                            router.push("/vendor/dashboard");
                        }
                    } else if (role === "vendor_staff" || role === "staff") {
                        if (status === "pending") {
                            // Automatically handled by the staff layout, but we'll push to staff
                            router.push("/staff");
                        } else {
                            router.push("/staff");
                        }
    ...
    return (
        <div className="min-h-screen bg-gray-50 flex items-center justify-center p-0 md:p-6 font-sans selection:bg-indigo-100">
            <div className="w-full max-w-md bg-white min-h-screen md:min-h-fit md:rounded-[2rem] md:shadow-2xl md:border border-gray-100 flex flex-col px-6 py-6 sm:py-8 mx-auto relative">
                <AuthSuccessModal
                    show={showSuccess}
                    autoRedirect={false} // Custom routing logic in handleSubmit
                    redirectPath="/"
                />
                ...
                <form className="space-y-5 w-full" onSubmit={handleSubmit}>
                    ...
                    <button
                        disabled={loading}
                        className={`w-full ${primaryBgGreen} text-white font-bold py-[18px] rounded-full shadow-lg ${hoverBgGreen} transition-all mb-6 mt-4 text-[16px] disabled:opacity-70 disabled:cursor-not-allowed`}
                    >
                ...
                <div className="text-center mt-auto pb-6">
                    ...
                    <span className="text-gray-500 text-[14px] font-medium">Don't have an account? </span>
                    <Link href="/signup" className={`text-[#16A34A] font-bold text-[14px] hover:underline ml-1`}>
                        Sign up
                    </Link>
                </div>
            </div>
