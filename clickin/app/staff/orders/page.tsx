...
import { subscribeToOrderHistory } from "@/lib/vendor-service"
...
export default function StaffOrdersPage() {
    ...
    useEffect(() => {
        if (!shopId) return
        setLoading(true)
        const unsubscribe = subscribeToOrderHistory(shopId, (history) => {
            setOrders(history)
            setLoading(false)
        }, "TODAY")
                                        ...
                                        <div>
                                            ...
                                            <div className="text-xs text-gray-500 font-medium flex items-center gap-3">
                                                <span className="flex items-center gap-1">
                                                    ...
                                                    {formatRelativeDate(order.createdAt)}, {new Date(order.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                                </span>
