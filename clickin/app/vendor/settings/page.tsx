...
export default function VendorSettingsPage() {
  ...
  const saveSettingsField = async (fields: Partial<VendorSettings> = {}) => {
    ...
    if (updatedSettings.upiId && !validateUpi(updatedSettings.upiId)) {
      setUpiError("Invalid UPI ID Format");
      return;
    }

    if (updatedSettings.famappUpiId && !validateUpi(updatedSettings.famappUpiId)) {
      setFamappUpiError("Invalid FamApp UPI ID Format");
      return;
    }

    setSaving(true);
    try {
      await updateSettings(shopId, updatedSettings);

      if (updatedSettings.upiId !== undefined) {
        await updateShop(shopId, { upiId: updatedSettings.upiId });
      }

      if (updatedSettings.famappUpiId !== undefined) {
        await updateShop(shopId, { famappUpiId: updatedSettings.famappUpiId });
      }

      // Keep local state in sync while Firestore listener updates too
          ...
          {tabs.map((t) => {
            ...
            return (
              ...
              >
                ...
                {active && (
                  <ChevronRight className="h-4 w-4 animate-pulse hidden lg:block" />
                )}
              </button>
            );
          })}
                ...
                <CardHeader className="bg-gray-900/5 p-8 border-b border-white/20">
                  <div className="flex items-center gap-4">
                    <div className="h-12 w-12 bg-white rounded-2xl shadow-sm flex items-center justify-center">
                      {ActiveIcon && (
                        <ActiveIcon className="h-6 w-6 text-gray-900" />
                      )}
                    </div>
                    <div>
                      ...
                      <p className="text-xs text-gray-500 font-medium">
                        Manage your {activeTab.replace("-", " ")} configuration
                      </p>
                    </div>
                  </div>
                </CardHeader>
                    ...
                    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {editedShop.cuisineType
                            .split(",")
                            .filter(Boolean)
                            .map((c, i) => (
                              <Badge
                                key={i}
                                className="bg-emerald-50 text-emerald-700 hover:bg-emerald-100 border-none px-3 py-1 font-bold text-[10px] uppercase"
                              >
                                {c.trim()}
                              </Badge>
                            ))}
                  ...
                  {activeTab === "hours" && (
                    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-500">
                      {(settings.operatingHours?.length
                        ...
                        <div
                          key={h.day}
                          className="group relative bg-white/40 p-5 rounded-3xl border border-white/40 hover:bg-white/80 transition-all hover:shadow-lg"
                        >
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                    ...
                    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-2 duration-500">
                      <div className="bg-emerald-50/50 rounded-[2rem] p-8 border border-emerald-100/50 relative overflow-hidden">
                        ...
                        <div className="flex items-center gap-3 mb-6">
                          ...
                          <h4 className="font-black text-emerald-900 tracking-tight text-lg uppercase">
                            UPI Configuration
                          </h4>
                        </div>
                        <div className="space-y-4 max-w-md">
                          ...
                          <p className="text-[10px] text-emerald-600/60 font-medium leading-relaxed bg-white/50 p-4 rounded-2xl">
                            💡 Use a verified merchant ID to avoid payout limits
                            and transaction failures on customer apps like GPay
                            or PhonePe.
                          </p>
                          
                          <div className="pt-4 border-t border-emerald-100/50">
                            <FloatingInput
                              label="FamApp UPI ID (Exclusive for FamApp Users)"
                              value={settings.famappUpiId || ""}
                              onChange={(
                                e: React.ChangeEvent<HTMLInputElement>,
                              ) => {
                                const v = e.target.value;
                                setSettings({ ...settings, famappUpiId: v });
                                markChanged();
                                setFamappUpiError(
                                  v && !validateUpi(v)
                                    ? "Invalid FamApp UPI ID Format"
                                    : null,
                                );
                              }}
                              onBlur={(e: React.FocusEvent<HTMLInputElement>) => {
                                ...
                                if (trimmed !== e.target.value) {
                                  setSettings((s) =>
                                    s ? { ...s, famappUpiId: trimmed } : null,
                                  );
                                }
                                saveSettingsField({ famappUpiId: trimmed });
                              }}
                              placeholder="famapp@upi"
                            />
                            {famappUpiError && (
                              <div className="flex items-center gap-2 text-red-500 text-xs font-bold animate-pulse mt-2">
                                <AlertCircle className="h-3.5 w-3.5" /> {famappUpiError}
                              </div>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <FloatingInput
                            ...
                            onChange={(
                              ...
                              setSettings((s) =>
                                s
                                  ? { ...s, bankAccountName: e.target.value }
                                  : null,
                              );
                              markChanged();
                            }}
                          ...
                          <FloatingInput
                            ...
                            onChange={(
                              ...
                              setSettings((s) =>
                                s
                                  ? { ...s, bankAccountNumber: e.target.value }
                                  : null,
                              );
                              markChanged();
                            }}
                          ...
                          <FloatingInput
                            ...
                            onChange={(
                              ...
                              setSettings((s) =>
                                s ? { ...s, bankIFSC: e.target.value } : null,
                              );
                              markChanged();
                            }}
                            onBlur={(e: React.FocusEvent<HTMLInputElement>) =>
                              saveSettingsField({ bankIFSC: e.target.value })
                            }
                          />
                          <FloatingInput
                            label="GST Number"
                            ...
                            onChange={(
                              ...
                              setSettings((s) =>
                                s ? { ...s, gstNumber: e.target.value } : null,
                              );
                              markChanged();
                            }}
                            onBlur={(e: React.FocusEvent<HTMLInputElement>) =>
                              saveSettingsField({ gstNumber: e.target.value })
                            }
                          />
                        </div>
                      </div>
                      ...
                      {[
                        {
                          ...
                          desc: "Push notification for every incoming order",
                          icon: Store,
                        },
                        {
                          key: "notifyOrderStatusChange" as const,
                          label: "Status Trackers",
                          desc: "Update when orders change states",
                          icon: Settings2,
                        },
                        {
                          key: "notifyLowStock" as const,
                          label: "Inventory Guard",
                          desc: "Low stock and out-of-stock alerts",
                          icon: AlertCircle,
                        },
                        {
                          key: "notifyDailySummary" as const,
                          label: "Analytics Pulse",
                          desc: "Receive automated daily performance stats",
                          icon: TrendingUp,
                        },
                        {
                          key: "orderAlertSound" as const,
                          label: "Audio Ping",
                          desc: "Loud alert sound for new orders (Dashboard)",
                          icon: Bell,
                        },
                      ].map((n) => (
                        <div
                          key={n.key}
                          className="flex items-center justify-between p-6 rounded-3xl bg-white/40 border border-white/40 hover:bg-white/70 transition-colors group"
                        >
                          <div className="flex items-center gap-4">
